...
public class ZMQ {
	...
	static {
		// if no embedded native library, revert to loading from
		// java.library.path
...
//			System.load("/usr/local/lib/libjzmq.so");
	...
	/**
	 * Old alias for DEALER flag. Flag to specify a XREQ socket, receiving side
	 * must be a XREP.
	 * 
	...
	/**
	 * Flag to specify ROUTER socket (aka XREP). ROUTER is the socket that
	 * creates and consumes request-reply routing envelopes. It is the only
	 * socket type that lets you route messages to specific connections if you
	 ...
	 * Subscriptions can be received as a message. Subscriptions start with a
	 * '1' byte. Unsubscriptions start with a '0' byte.
	 */
	...
	/**
	 * @return Full version number of the ZMQ library used for comparing
	 *         versions.
	 */
	 ...
	 */
	public static int makeVersion(final int major, final int minor,
			final int patch) {
	...
	public static String getVersionString() {
		return String.format("%d.%d.%d", version_major(), version_minor(),
				version_patch());
	}
	...
	public enum Error {
		...
		public static Error findByCode(int code) {
			...
			throw new IllegalArgumentException("Unknown "
					+ Error.class.getName() + " enum code:" + code);
		}
	}
	...
	public static class Context {
		...
		/**
		 * Create a new Poller within this context, with a specified initial
		 * size.
		 * 
		 * @param size
	...
	public static class Socket {
		 ...
		 * 0, recv will return immediately, with a EAGAIN error if there is no
		 * message to receive. If the value is -1, it will block until a message
		 * is available. For all other values, it will wait for a message for
		 ...
		 * send will return immediately, with a EAGAIN error if the message
		 * cannot be sent. If the value is -1, it will block until the message
		 * is sent. For all other values, it will try to send the message for
		 * that amount of time before returning with an EAGAIN error.
		 * 
		 * @param timeout
		 */
		...
		/**
		 * The 'ZMQ_RCVMORE' option shall return a boolean value indicating if
		 * the multi-part message currently being read from the specified
		 * 'socket' has more message parts to follow. If there are no message
		 * parts to follow or if the message currently being read is not a
		 * multi-part message a value of zero shall be returned. Otherwise, a
		...
		/**
		 * The 'ZMQ_FD' option shall retrieve file descriptor associated with
		 * the 0MQ socket. The descriptor can be used to integrate 0MQ socket
		 * into an existing event loop. It should never be used for anything
		 * else than polling -- such as reading or writing. The descriptor
		 * signals edge-triggered IN event when something has happened within
		 * the 0MQ socket. It does not necessarily mean that the messages can be
		 * read or written. Check ZMQ_EVENTS option to find out whether the 0MQ
		 ...
		 * socket. If a message can be read from the socket ZMQ_POLLIN flag is
		 * set. If message can be written to the socket ZMQ_POLLOUT flag is set.
		 * 
		...
		/**
		 * The 'ZMQ_LINGER' option shall retrieve the period for pending
		 * outbound messages to linger in memory after closing the socket. Value
		 * of -1 means infinite. Pending messages will be kept until they are
		 * fully transferred to the peer. Value of 0 means that all the pending
		 * messages are dropped immediately when socket is closed. Positive
		 * value means number of milliseconds to keep trying to send the pending
		 ...
		 * 'socket'. The high water mark is a hard limit on the maximum number
		 * of outstanding messages 0MQ shall queue in memory for any single peer
		 ...
		 * individual socket descriptions in the man page of zmq_socket[3] for
		...
		/**
		 * Get the Swap. The 'ZMQ_SWAP' option shall set the disk offload (swap)
		 * size for the specified 'socket'. A socket which has 'ZMQ_SWAP' set to
		 * a non-zero value may exceed its high water mark; in this case
		 * outstanding messages shall be offloaded to storage on disk rather
		 * than held in memory.
		 * 
		 * @param swap
		 *            The value of 'ZMQ_SWAP' defines the maximum size of the
		 *            swap space in bytes.
		 */
		 ...
		 * affinity for newly created connections on the specified 'socket'.
		 * 
		 * Affinity determines which threads from the 0MQ I/O thread pool
		 * associated with the socket's _context_ shall handle newly created
		 ...
		 * 
		 * See also in the man page of zmq_init[3] for details on allocating the
		 * number of I/O threads for a specific _context_.
		 * 
		 * @param affinity
		 *            the affinity.
		 */
		 ...
		 * keep-alive packets for a socket connection. Possible values are -1,
		 * 0, 1. The default value -1 will skip all overrides and do the OS
		 * default.
		 * 
		 * @param optVal
		 *            The value of 'ZMQ_TCP_KEEPALIVE' to turn TCP keepalives on
		 *            (1) or off (0).
		 */
		...
		/**
		 * Override TCP_KEEPCNT socket option (where supported by OS). The
		 ...
		 * 
		 * @param optVal
		 *            The value of 'ZMQ_TCP_KEEPALIVE_INTVL' defines the
		 *            interval between keepalives in ms.
		 */
		 ...
		 * (where supported by OS). The default value -1 will skip all overrides
		 * and do the OS default.
		 * 
		 * @param optVal
		 *            The value of 'ZMQ_TCP_KEEPALIVE_IDLE' defines the interval
		 *            between the last data packet sent over the socket and the
		 *            first keepalive probe in ms.
		 */
		 ...
		 * 'socket'. Socket identity determines if existing 0MQ infastructure
		 * (_message queues_, _forwarding devices_) shall be identified with a
		 ...
		 * socket shall re-use any existing 0MQ infrastructure configured by the
		 * previous run(s). Thus the application may receive messages that were
		 * sent in the meantime, _message queue_ limits shall be shared with
		 ...
		 * messages beginning with the specified prefix. Mutiple filters may be
		...
		/**
		 * The 'ZMQ_UNSUBSCRIBE' option shall remove an existing message filter
		 * on a 'ZMQ_SUB' socket. The filter specified must match an existing
		 * filter previously established with the 'ZMQ_SUBSCRIBE' option. If the
		 * socket has several instances of the same filter attached the
		 * 'ZMQ_UNSUBSCRIBE' option shall remove only one instance, leaving the
		...
		/**
		 * The 'ZMQ_RATE' option shall set the maximum send or receive data rate
		 * for multicast transports such as in the man page of zmq_pgm[7] using
		 ...
		 * multicast transports using the specified 'socket'. The recovery
		 * interval determines the maximum time in seconds that a receiver can
		 * be absent from a multicast group before unrecoverable data loss will
		 * occur.
		 * 
		 * CAUTION: Excersize care when setting large recovery intervals as the
		 * data needed for recovery will be held in memory. For example, a 1
		 * minute recovery interval at a data rate of 1Gbps requires a 7GB
		 * in-memory buffer. {Purpose of this Method}
		 * 
		 * @param recovery_ivl
		 */
		 ...
		 * multicast transports using the specified 'socket' can also be
		 * received by the sending host via loopback. A value of zero disables
		 * the loopback functionality, while the default value of 1 enables the
		 * loopback functionality. Leaving multicast loopback enabled when it is
		 * not required can have a negative impact on performance. Where
		 * possible, disable 'ZMQ_MCAST_LOOP' in production environments.
		 * 
		 * @param mcast_loop
		 */
		...
		/**
		 * The 'ZMQ_SNDBUF' option shall set the underlying kernel transmit
		 * buffer size for the 'socket' to the specified size in bytes. A value
		 * of zero means leave the OS default unchanged. For details please
		 * refer to your operating system documentation for the 'SO_SNDBUF'
		 * socket option.
		 * 
		 * @param sndbuf
		 */
		...
		/**
		 * The 'ZMQ_RCVBUF' option shall set the underlying kernel receive
		 * buffer size for the 'socket' to the specified size in bytes. A value
		 * of zero means leave the OS default unchanged. For details refer to
		 * your operating system documentation for the 'SO_RCVBUF' socket
		 * option.
		 * 
		 * @param rcvbuf
		 */
		 ...
		 * @param len
		 *            max bytes to write to buffer. If len is smaller than the
		 *            incoming message size, the message will be truncated.
		 * @param flags
		 *            the flags to apply to the receive operation.
		...
		/**
		 * Get the underlying socket handle. This is private because it is only
		 * accessed from JNI, where Java access controls are ignored.
		 * 
	...
	public static class Poller {
		...
		/**
		 * Issue a poll call. If the poller's internal timeout value has been
		 * set, use that value as timeout; otherwise, block indefinitely.
		 * 
		 ...
		 * <p>
		 * Since ZeroMQ 3.0, the timeout parameter is in <i>milliseconds<i>, but
		 * prior to this the unit was <i>microseconds</i>.
		 * 
		 * @param tout
		 *            the timeout, as per zmq_poll (); if -1, it will block
		 *            indefinitely until an event happens; if 0, it will return
		 *            immediately; otherwise, it will wait for at most that many
		...
		public long poll(long tout) {
			...
			return run_poll(this.used, this.sockets, this.events, this.revents,
					tout);
		}

		/**
		 * Check whether the specified element in the poll set was signalled for
		 * input.
		 * 
		 * @param index
		 * 
		 ...
		 * @param timeout
		 *            the maximum timeout in milliseconds/microseconds (see
		 *            above).
		 * @return how many objects where signalled by poll ().
		 * @see http://api.zeromq.org/2-1:zmq-poll
		 * @see http://api.zeromq.org/3-0:zmq-poll
		 */
		private native long run_poll(int count, Socket[] sockets,
				short[] events, short[] revents, long timeout);
